...
const formSchema = z.object({
  name: z
    ...
    .max(100, { message: "O nome da loja deve ter no maximo 100 caracteres" }),
});
...
export const SettingsForm: React.FC<SettingsFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (data: SettingsFormValues) => {
    try {
      ...
      router.refresh();
      toast.success("Store update (Loja atualizada)")
    } catch (error) {
  ...
  return (
    ...
    />
      <div className="flex items-center justify-between">
        <Heading title="Configurações da loja" description="Gerenciador de preferências" />
        <Button
          disabled={loading}
          variant="destructive"
          size="sm"
